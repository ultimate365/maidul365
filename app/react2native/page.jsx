...
const ReactToReactNativeConverter = () => {
  ...
  const convertClassNameToStyle = (classNameString) => {
    ...
    classes.forEach((className) => {
      ...
      if (styleMappings.tailwind[className]) {
        Object.assign(styleObject, styleMappings.tailwind[className]);
        hasTailwind = true;
      }
      // Check for Bootstrap classes
      else if (styleMappings.bootstrap[className]) {
        Object.assign(styleObject, styleMappings.bootstrap[className]);
        hasBootstrap = true;
      }
      // Check for responsive classes (like md:, sm:)
      else if (className.includes(":")) {
        const [breakpoint, actualClass] = className.split(":");
        if (styleMappings.tailwind[actualClass]) {
          Object.assign(styleObject, styleMappings.tailwind[actualClass]);
          hasTailwind = true;
        } else {
  ...
  const convertCode = async (code = inputCode) => {
    ...
    const stylesForSheet = {};
    let styleCounter = 1;

    // Component conversion rules
    const componentRules = [
      // Specific conversions first
      {
        regex: /<option\s+value=["']([^"']+)["'][^>]*>([^<]*)<\/option>/g,
        replacement: '<Picker.Item label="$2" value="$1" />',
      },
      {
        regex: /<select\b([^>]*)>/g,
        replacement: "<Picker$1>",
      },
      {
        regex: /<\/select>/g,
        replacement: "</Picker>",
      ...
      { regex: /<label\b([^>]*)>/g, replacement: "<Text$1>" },
      { regex: /<\/label>/g, replacement: "</Text>" },
      { regex: /<label\b([^>]*)>/g, replacement: "<Text$1>" },
      { regex: /<\/label>/g, replacement: "</Text>" },
      { regex: /<h1\b([^>]*)>/g, replacement: "<Text$1>" },
      { regex: /<\/h1>/g, replacement: "</Text>" },
      { regex: /<h2\b([^>]*)>/g, replacement: "<Text$1>" },
      { regex: /<\/h2>/g, replacement: "</Text>" },
      { regex: /<h3\b([^>]*)>/g, replacement: "<Text$1>" },
      { regex: /<\/h3>/g, replacement: "</Text>" },
      { regex: /<h4\b([^>]*)>/g, replacement: "<Text$1>" },
      { regex: /<\/h4>/g, replacement: "</Text>" },
      { regex: /<h5\b([^>]*)>/g, replacement: "<Text$1>" },
      { regex: /<\/h5>/g, replacement: "</Text>" },
      { regex: /<h6\b([^>]*)>/g, replacement: "<Text$1>" },
      { regex: /<\/h6>/g, replacement: "</Text>" },
      { regex: /<button\b([^>]*)>/g, replacement: "<TouchableOpacity$1>" },
      { regex: /<\/button>/g, replacement: "</TouchableOpacity>" },
      {
        regex: /<input\b[^>]*type=["']date["'][^>]*>/g,
        replacement: `\n{/* TODO: Implement state and open handler for DateTimePicker\n<DateTimePicker value={date} mode="date" display="default" onChange={onDateChange} />\n*/}`,
      },
      ...
      // Event handlers
      // Convert onChange handlers: (e) => { ... e.target.value ... } to (value) => { ... value ... }
      {
        regex: /onChange=\{?\s*\(\s*(\w+)\s*\)\s*=>\s*([\s\S]+?)\s*\}/g,
        replacement: (match, eventParam, functionBody) => {
          // Replace all instances of `e.target.value` with `value`
          const newBody = functionBody
            .replace(
              new RegExp(`\\b${eventParam}\\.target\\.value\\b`, "g"),
              "value"
            )
            .trim();
          // Return the new onChangeText prop with the updated handler
          return `onChangeText={(value) => ${newBody}}`;
        },
      },
    ...
    let match;
    let tempConvertedCode = convertedCode;

    while ((match = classRegex.exec(tempConvertedCode)) !== null) {
      const fullMatch = match[0];
      const classes = match[2];

      const conversion = convertClassNameToStyle(classes);

      stats.tailwindClasses += conversion.hasTailwind ? 1 : 0;
      stats.bootstrapClasses += conversion.hasBootstrap ? 1 : 0;
      ...
      if (Object.keys(conversion.styleObject).length > 0) {
        ...
        stylesForSheet[styleName] = conversion.styleObject;

        let replacement = `style={styles.${styleName}}`;
    ...
    // Remove any remaining empty className attributes
    convertedCode = convertedCode.replace(/\s*className=["']["']\s*/g, " ");
    ...
    if (
      convertedCode.includes("<TouchableOpacity") ||
      convertedCode.includes("</TouchableOpacity>")
    ) {
    ...
    if (
      convertedCode.includes("<ScrollView") ||
      convertedCode.includes("</ScrollView>")
    ) {
      componentsToImport.push("ScrollView");
    }
    if (convertedCode.includes("<Picker")) {
      if (
        !convertedCode.includes(
          "import { Picker } from '@react-native-picker/picker'"
        )
      ) {
        convertedCode = `import { Picker } from '@react-native-picker/picker';\n${convertedCode}`;
      }
    }
    if (convertedCode.includes("<DateTimePicker")) {
      if (
        !convertedCode.includes(
          "import DateTimePicker from '@react-native-community/datetimepicker'"
        )
      ) {
        convertedCode = `import DateTimePicker from '@react-native-community/datetimepicker';\n${convertedCode}`;
      }
    }

    if (
      componentsToImport.length > 0 &&
      !convertedCode.includes("from 'react-native'")
    ) {
      const importStatement = `import { ${componentsToImport.join(
        ", "
      )} } from 'react-native';\n`;
      ...
      if (matches) {
        ...
        const lastImportIndex =
          convertedCode.lastIndexOf(lastImport) + lastImport.length;
        convertedCode =
          convertedCode.slice(0, lastImportIndex) +
          "\n" +
          importStatement +
          convertedCode.slice(lastImportIndex);
      } else {
      ...
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-8">
          ...
          <p className="text-gray-400">
            Convert React.js components with Bootstrap/Tailwind CSS to React
            Native
          </p>
        </header>
          ...
          <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-blue-900/50 p-4 rounded-lg border border-blue-700 text-center">
              <div className="text-2xl font-bold text-blue-400">
                {conversionStats.tailwindClasses || 0}
              </div>
              <h2 className="text-sm text-blue-300">Tailwind Classes</h2>
            </div>
            <div className="bg-purple-900/50 p-4 rounded-lg border border-purple-700 text-center">
              <div className="text-2xl font-bold text-purple-400">
                {conversionStats.bootstrapClasses || 0}
              </div>
              <h2 className="text-sm text-purple-300">Bootstrap Classes</h2>
            </div>
            <div className="bg-green-900/50 p-4 rounded-lg border border-green-700 text-center">
              <div className="text-2xl font-bold text-green-400">
                {conversionStats.componentsConverted || 0}
              </div>
              <h2 className="text-sm text-green-300">Components Converted</h2>
            </div>
            <div className="bg-yellow-900/50 p-4 rounded-lg border border-yellow-700 text-center">
              <div className="text-2xl font-bold text-yellow-400">
                {conversionStats.unconvertedClasses || 0}
              </div>
              <h2 className="text-sm text-yellow-300">Unconverted Classes</h2>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700">
            <div className="bg-gray-750 px-4 py-3 flex justify-between items-center border-b border-gray-700">
              ...
              <span className="text-xs bg-blue-500 px-3 py-1 rounded-full font-mono">
                JSX/TSX
              </span>
            </div>
            <div className="h-96">
          ...
          <div className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700">
            <div className="bg-gray-750 px-4 py-3 flex justify-between items-center border-b border-gray-700">
              ...
              <span className="text-xs bg-green-500 px-3 py-1 rounded-full font-mono">
                React Native
              </span>
            </div>
            <div className="h-96">
