...
function Solver(props) {
  ...
  useEffect(() => {
      ...
      for (var i = 0; i < 6 * times; i++) {
        ...
        for (var j = 0; j < 5; j++) {
          var sticker1 = piece1.children[j < 4 ? mx(face, j) : face].firstChild,
            sticker2 =
              piece2.children[j < 4 ? mx(face, j + 1) : face].firstChild,
    ...
    function animateRotation(face, cw, currentTime, async = true) {
      ...
      if (async) {
        ...
        (function rotatePieces() {
          var style = "rotate" + getAxis(face) + "(" + 300;
          ("deg)");
          qubes.forEach(function (piece) {
            piece.style.transform = piece.style.transform.replace(
              /rotate.\(\S+\)/,
    ...
    function scramble_cube() {
      ...
      setTimeout(() => {
        for (
          reverseIndex = 0;
          reverseIndex <= props.movesAlgo.reverseAlgo.length - 1;
          reverseIndex++
        ) {
    ...
    function handleKeys(e) {
      ...
      if (inputKey == "ArrowLeft" && !prevBtn.hasAttribute("disabled")) {
        ...
        inputKey == "ArrowDown" &&
        !repeatBtn.hasAttribute("disabled")
      ) {
        repeatMove();
      } else if (
        inputKey == "ArrowRight" &&
  ...
  return (
    <div className={`cube__container${solverStyle}`}>
      ...
      <div className="scramble__error">
        Memory consumption is high. Please try again!
      </div>
